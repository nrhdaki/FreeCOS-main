...
class LSystem_vessel():
    def __init__(self, axiom, rules, rules_2, rules_3, rules_4, rules_5 ,rules_6, rules_7, rules_8, theta=0, width=5, dtheta_1=40, dtheta_2=30, start=(-350,0), length=80,iteration=3,width_1=0.79, width_2 = 0.5):
        self.sentence = axiom
        self.rules = rules
            ...
            for char in self.sentence:
                ...
                try:
                    ...
                    if p>0.7:
                        ...
                        mapped = self.rules_2[char]
                    elif 0.5>p>=0.6:
                        mapped = self.rules_3[char]
                    elif 0.45>p>=0.5:
                        mapped = self.rules_4[char]
                    elif 0.40>p>=0.45:
                        mapped = self.rules_5[char]
                    elif 0.20>p>=0.40:
                        mapped = self.rules_6[char]
                    elif 0.10>p>=0.20:
                        mapped = self.rules_7[char]
                    else:
                        mapped = self.rules_8[char]
                except:
                    pass
                newStr += mapped
    ...
    def draw(self, turtle):
        ...
        for char in self.sentence:
            ...
            if char == 'F' or char == 'G':
                ...
                dtheta = np.random.randint(low=10, high=40)
                randangle = np.random.uniform(-0.1 * dtheta, 0.1 * dtheta)
                self.theta += 0.5 * dtheta + randangle
                self.x += 0.5 * dtheta + randangle
                turtle.setheading(self.theta)
            elif char == '*':  # Steering operation
                dtheta = np.random.randint(low=10, high=40)
                randangle = np.random.uniform(-0.1 * dtheta, 0.1 * dtheta)
                self.theta += 0.5 * dtheta + randangle
                turtle.right(0.5 * dtheta + randangle)
                self.y -= 0.5 * dtheta + randangle
            elif char == 'J':  # Jumping operation
                d = np.random.uniform(-0.5 * self.length, 0.5 * self.length)  # Random distance within [-0.5d, +0.5d]
                rand_angle = np.radians(self.theta)
                dx = d * np.sin(rand_angle)
                dy = d * np.cos(rand_angle)
                self.x += dx
                self.y += dy
                turtle.pu()
                turtle.goto((self.x, self.y))
...
rules_2 =  {"F":"-F-F+++F+F-F-"}
rules_3 =  {"F":"+F+F----F+F+"}
rules_4 =  {"F":"-F**F$$F+$$F$$F-F"}
rules_5 =  {"F":"+$F+F-**F***F**F$$F"}
rules_6 =  {"F":"++F---F-FF++++FF--"}
rules_7 =  {"F":"+F--F[-F+F-F]++F"}
rules_8 =  {"F":"**F$$$F[++F**F$F]**F"}
'''
...
for i  in range(150,150+Num_image):
    ...
    system = LSystem_vessel(path, r1, r2, r3 ,r4  ,theta=init_theta, width=init_width, dtheta_1=dtheta_1, dtheta_2=dtheta_2, start=(x_position, y_position),length=init_length, iteration=iteration, width_1=Ratio_lw_1, width_2=Ratio_lw_2)
    system.generate()
    turtle.speed(0)
    turtle.delay(0)
    turtle.tracer(False)
