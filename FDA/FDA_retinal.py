...
def _rotate_and_crop(image, output_height, output_width, rotation_degree, do_crop):
    ...
    Args:
        ...
        do_crop: Do cropping(是否裁剪) if it is True.
    Returns:
        A rotated image.
    """
...
def add_gaussian_noise(X_imgs): 
    ...
    if len(X_imgs.shape) == 2: #判断维度是否为2（即灰度图像）
    ...
    cv2.normalize(noisy_image, noisy_image, 0, 255, cv2.NORM_MINMAX, dtype=-1) #归一化处理
    noisy_image = noisy_image.astype(np.uint8)
    return noisy_image

def _largest_rotated_rect(w, h, angle):
    # 计算中心最大矩形区域，可以帮助定位目标对象的准确位置，帮助提取图像中的主要信息和特征
    """
    ...
    quadrant = int(math.floor(angle / (math.pi / 2))) & 3 #判断象限
    sign_alpha = angle if ((quadrant & 1) == 0) else math.pi - angle
    alpha = (sign_alpha % math.pi + math.pi) % math.pi

    bb_w = w * math.cos(alpha) + h * math.sin(alpha) #计算宽度和高度
...
#将图像中指定的灰度颜色值替换为另一种灰度颜色值：去除噪点无效信息、调整图像表现、目标检测分割
