...
#从复数的FFT的表示中提取振幅和相位信息（傅里叶变换）
def extract_ampl_phase(fft_im):
    # fft_im: size should be bx3xhxwx2 b：batch size/3：通道数/h：高度/w：宽度
    fft_amp = fft_im[:,:,:,:,0]**2 + fft_im[:,:,:,:,1]**2 #提取实部和虚部部分
    fft_amp = torch.sqrt(fft_amp) #得到振幅值
    fft_pha = torch.atan2( fft_im[:,:,:,:,1], fft_im[:,:,:,:,0] ) #得到相位信息
