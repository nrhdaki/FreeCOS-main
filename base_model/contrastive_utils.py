...
import torch
import random
from scipy.ndimage.morphology import distance_transform_edt #计算图像的欧式距离变换
import torch.nn.functional as F #各种神经网络函数
import PIL.Image as Image #图像处理库
random.seed(1)


def enhance_op(x): #增强操作的函数
    assert x.dim() == 4, f'only support BxCxHxW which dim=4' #接受四维张量作为输入
    N, C, H, W = x.shape
    phi_x = F.normalize(x, p=2, dim=1, eps=1e-12, out=None).view(N, C, -1) #L2标准化
    theta_x = F.normalize(x, p=2, dim=1, eps=1e-12, out=None).view(N, C, -1).permute(0, 2, 1) #调整形状
    pairwise_weight = torch.matmul(theta_x, phi_x)
    pairwise_weight = pairwise_weight.softmax(dim=-1) #计算点乘、归一化，得到权重函数
    x = x.view(N, C, -1).permute(0, 2, 1) #调整形状
    out = torch.matmul(pairwise_weight, x) #输出张量
    out = out.permute(0, 2, 1).contiguous().reshape(N, C, H, W) #进行维度调整

    return out


def normalize_batch(cams_batch): #归一化张量的函数
    """
    Classic min-max normalization
    """
    bs = cams_batch.size(0)
    cams_batch = cams_batch + 1e-4 #避免出现除零错误
    cam_mins = getattr(cams_batch.view(bs, -1).min(1), 'values').view(bs, 1, 1, 1) #存储最大最小值
    cam_maxs = getattr(cams_batch.view(bs, -1).max(1), 'values').view(bs, 1, 1, 1)
    return (cams_batch - cam_mins) / (cam_maxs - cam_mins) #将元素缩放到0-1之间输出


def get_query_keys_eval(cams):#处理输入张量并返回查询键值集合
    """
        Input
    ...
    """
    return_result = dict()
    cams = cams.squeeze(1).cpu() #将大小为1的维度去除
    cams = normalize_zero_to_one(cams)  # tensor  shape:N,28,28, 0-1

    # we only need queries
    query_pos_sets = torch.where(cams > 0.92, 1.0, 0.0).to(dtype=torch.bool) #大于0.91设为1
    query_neg_sets = torch.where(cams < 0.08, 1.0, 0.0).to(dtype=torch.bool) #小于0.08设为0，并转化为布尔类型张量

    return_result['query_pos_sets'] = query_pos_sets #存储到字典
...
def get_query_keys(
    ...
    cams = normalize_zero_to_one(cams)  # normalize to 0~1归一化

    #######################################################
    # ---------- get query mask for each proposal ----------
    #######################################################
    query_pos_sets = masks.to(dtype=torch.bool)  # here, pos=foreground area  neg=background area mask:0忽略1前景目标-1背景
    ...
    #正例掩码:与标签数据形状相同的张量，其中True值表示正类样本，False 值表示负类样本。正例掩码主要用于计算分类问题中的损失函数
    #负例掩码:与标签数据形状相同的张量, True 值表示负类样本，False 值表示正类样本。负例掩码通常用于处理类别不平衡的问题
    #换成CAM（类激活图）：用于解释卷积神经网络模型中特定类别的分类依据和区域关注
    #过滤无效样本
    ...
    # For novel(unseen)未见过的样本, replace query mask via cam's confidence since query mask are unknown for novel(unseen)
    #利用 cams 的置信度信息来替换未知样本的查询掩码，从而更好地适应和处理那些不熟悉的样本
    ...
    edge_sets_dilate = get_pixel_sets_distrans(edges, radius=2)  # expand edges with radius=2扩展边缘
    ...
    hard_negative_sets = torch.where((hard_pos_neg_sets - masks) > 0.5, 1.0, 0.0) #1难负例
    hard_positive_sets = torch.where((hard_pos_neg_sets - hard_negative_sets) > 0.5, 1.0, 0.0) #1难正例
    easy_positive_sets = torch.where((masks - hard_pos_neg_sets) > 0.5, 1.0, 0.0) #1容易正例
    easy_negative_sets = torch.logical_not(torch.where((masks + edge_sets_dilate) > 0.5, 1.0, 0.0)).to(
        dtype=easy_positive_sets.dtype) #1容易负例
    ...
    easy_positive_sets[is_novel] = unseen_positive_sets[is_novel] #将未知类别的正例和负例替换到原始的 easy_positive_sets等标签
    ...
    #处理未知类别样本，将其正例和负例信息整合到模型已有的数据集中
    ...
    if True not in keeps:  # all proposals do not have enough points that can be sample. This is a extreme situation.
        #根据 keeps 张量中的信息来确定每种类型集合中需要采样的点数，并且在极端情况下（所有提案物足够采样点）将点数设置为2
    ...
    # ----------------- sample points ---------------------
    #######################################################
    #根据之前计算得到的需要采样的点数，从对应的集合中获取相应数量的像素点集合
    ...
    return_result['num_per_type'] = num_per_type #各种类型点数信息
...
#根据输入的边缘和掩码信息，以及其他参数，计算并返回查询关键点
def get_query_keys_sty(
    ...
    # Record points number检查和记录采样后的点集合情况
    num_per_type = dict()
    num_per_type['Epos_num_'] = sample_num_Epos
    num_per_type['Hpos_num_'] = sample_num_Hpos
    num_per_type['Eneg_num_'] = sample_num_Eneg
...
def get_query_keys_myself(
    ...
    num_Hpos_ = hard_positive_sets.sum(dim=[2, 3]) #dim=[2, 3]表示对第二个和第三个维度进行求和
    num_Eneg_ = easy_negative_sets.sum(dim=[2, 3])
    num_Hneg_ = hard_negative_sets.sum(dim=[2, 3])
    # print("num_Epos_",num_Epos_)#
    # print("num_Hpos_",num_Hpos_)
    ...
    # print("num_Hneg_",num_Hneg_.shape)#8 1
    # if available points are less then 5 for each type, this proposal will be dropped out.
    available_num = torch.cat([num_Epos_, num_Eneg_, num_Hpos_, num_Hneg_]) #拼接张量的第一维
    # print("available_num",available_num.shape)#32 , 1
    # print("available_num", available_num)#4 , 8
    abandon_inds = torch.where(available_num < 5, 1, 0).reshape(4, -1)
    # print("abandon_inds", abandon_inds.shape)#4 , 8
    # print("abandon_inds", abandon_inds)#4 , 8
    keeps = torch.logical_not(abandon_inds.sum(0).to(dtype=torch.bool)) #按列求和，将结果取反，得到长度为batch_size的布尔类型张量，表示哪些proposal应该保留
    # print("keeps",keeps.shape)#8
...
def get_pixel_sets_N(src_sets, select_num):
    ...
    if isinstance(src_sets, torch.Tensor):
        ...
        flag = True
        if torch.where(src_sets > 0.5, 1, 0).shape[0]==0: #将大于0.5的值转换为1，小于等0.5的值转换为0，并将结果展平成一维数组
            flag = False
...
def get_pixel_sets_N_myself(src_sets, select_num): #根据预设条件筛选并随机选择像素点，形成新的像素集合数据，并返回处理后的结果和处理状态
    return_ = []
    if isinstance(src_sets, torch.Tensor):
        bs, c,h, w = src_sets.shape
        #print("src_sets_shape",src_sets.shape)#5 1 256 256
...
#对输入的图像集合进行欧几里得距离变换，将每个像素点替换为其到最近的背景像素点的距离，根据阈值radius得到二值化的像素集合结果
...
def mask2edge(seg): #将输入的分割掩模转换为边缘检测结果
    ...
    edge_targets[edge_targets <= 0.1] = 0 #二值化检测结果
    return edge_targets
